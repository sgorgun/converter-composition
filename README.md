...
Solve the converting problem the real number to its "in words" string representations in any given language. 
...
- Implement [Transformer](TransformerDictionaryComposition/Transformer) class that converts real number to its "word format" in any given language. String presentation depend on the language and the set of words that are presented by a `СharactersDictionary` class. The `Transformer` class _controls the lifetime_ of an `CharsDictionary` class object (composition), but it do it with the help of an additional abstraction - an factory class.
